...
if (!document.getElementById("sbm-fonts")) {
  ...
  l.id = "sbm-fonts"; l.rel = "stylesheet";
  l.href = "https://fonts.googleapis.com/css2?family=JetBrains+Mono:wght@300;400;500;600;700&family=DM+Sans:ital,wght@0,300;0,400;0,500;0,600;1,400&family=Fraunces:ital,wght@0,300;0,400;1,300;1,400&display=swap";
...
/* ══════════════════════════════════════════════════════════════
   HELPERS
══════════════════════════════════════════════════════════════ */
function adcToVolt(v) { return ((v / 4095) * 3.3).toFixed(3); }
function calcFreq(psc, per) { return (16_000_000 / ((psc + 1) * (per + 1))).toFixed(2); }
function calcDuty(pulse, period) { return ((pulse / (period + 1)) * 100).toFixed(1); }
...
function PWMWave({ duty, freq }) {
  const cycles = 3, W = 400, H = 78, pad = 10;
  ...
  const fills = Array.from({length: cycles}, (_, i) => {
    const x0 = pad + i*cw;
    return <rect key={i} x={x0} y={hi} width={onW} height={lo-hi} fill="url(#wg)"/>;
  });
  return (
    <svg width="100%" height={H} viewBox={`0 0 ${W} ${H}`} style={{display:"block"}}>
      ...
      <line x1={pad} y1={hi} x2={W-pad} y2={hi} stroke="#0f1a12" strokeWidth="1" strokeDasharray="3 4"/>
      <line x1={pad} y1={lo} x2={W-pad} y2={lo} stroke="#0f1a12" strokeWidth="1" strokeDasharray="3 4"/>
      {fills}
      <path d={d} fill="none" stroke="#22c55e" strokeWidth="1.5" strokeLinejoin="round"/>
      <text x={pad+3} y={hi-3} fontSize="9" fill="#3a5443" fontFamily="JetBrains Mono,monospace">HIGH</text>
      <text x={pad+3} y={lo+10} fontSize="9" fill="#3a5443" fontFamily="JetBrains Mono,monospace">LOW</text>
      <text x={W-pad-2} y={H-1} fontSize="9" fill="#2d4035" fontFamily="JetBrains Mono,monospace" textAnchor="end">{freq} Hz</text>
    </svg>
  );
}
...
function RingGauge({ value, max=100, color="#22c55e", size=120 }) {
  ...
  return (
    <svg width={size} height={size} viewBox="0 0 120 120">
      <circle cx="60" cy="60" r="44" fill="none" stroke="#0f1a12" strokeWidth="7"/>
      <circle cx="60" cy="60" r="44" fill="none" stroke={color} strokeWidth="7"
        strokeLinecap="round" strokeDasharray={circ} strokeDashoffset={offset}
        transform="rotate(-90 60 60)"
        style={{transition:"stroke-dashoffset .4s cubic-bezier(.4,0,.2,1)", filter:`drop-shadow(0 0 6px ${color}55)`}}/>
    </svg>
  );
}
...
function ChartTip({ active, payload }) {
  ...
  return (
    <div style={{background:"#0d1510",border:"1px solid #172016",borderRadius:6,padding:"6px 10px",fontFamily:"'JetBrains Mono',monospace",fontSize:11}}>
      <div style={{color:"#22c55e"}}>{Math.round(v)} <span style={{color:"#3a5443",fontSize:9}}>ADC</span></div>
      <div style={{color:"#f59e0b",fontSize:10}}>{adcToVolt(v)} V</div>
    </div>
  );
}
...
function WaitingOverlay({ error, wsState }) {
  const wsLabel = { connecting:"Menyambung ke backend…", open:"Backend terhubung", closed:"Backend tidak aktif", error:"Koneksi backend gagal" };
  ...
  return (
    <div className="wait-overlay">
      ...
      <div className="wait-sub">
        Tancapkan kabel USB ST-Link ke laptop/PC.<br/>
      ...
      <div className="wait-ws-status">
        {wsLabel[wsState] ?? wsState}
      </div>
    </div>
  );
}
...
/* ══════════════════════════════════════════════════════════════
   DASHBOARD
══════════════════════════════════════════════════════════════ */
const DEFAULT_HIST = () => Array.from({length:HIST_LEN}, (_,i) => ({t:i, v:0}));
...
function Dashboard({ onGoToDocs }) {
    ...
    ws.onmessage = (e) => {
      try {
        ...
        if (d.connected) {
          ...
          setAdc(v);
          setPsc(d.psc   ?? 15999);
          setPeriod(d.period ?? 499);
          setPulse(d.pulse  ?? 250);
          setUptime(d.uptime ?? 0);

          /* Rolling history */
          setHist(h => {
            const next = [...h.slice(1), {t: tick.current++, v}];
            setAdcStats({
  ...
  const css = `
    ...
    @media(max-width:860px){.g3{grid-template-columns:1fr 1fr}.g3b{grid-template-columns:1fr 1fr}}
    @media(max-width:580px){.g3,.g3b{grid-template-columns:1fr}}
  `;

  /* ── Chip status ── */
  const chipClass = connected ? "chip-green" : wsState === "closed" || wsState === "error" ? "chip-offline" : "chip-waiting";
  const chipLabel = connected ? "CONNECTED" : wsState === "closed" || wsState === "error" ? "BACKEND OFFLINE" : "WAITING…";
  ...
  return (
    <>
      ...
      <div className="db-wrap page-enter" style={!connected ? {filter:"blur(1.5px) grayscale(.4)", pointerEvents:"none", userSelect:"none"} : {}}>

        {/* Header */}
        <header className="hdr">
          <div className="hdr-ident">
            <div className={`hdr-dot${connected?"":""}`} style={connected ? {} : {background:"var(--amber)",boxShadow:"0 0 10px var(--amber)"}}/>
            <div>
              <div className="hdr-title">SBM Monitor</div>
              <div className="hdr-sub">sbmproject · dnday · STM32F401CCUx · Live via ST-Link</div>
            </div>
          </div>
          <div className="hdr-right">
            <div className="badge">MCU <span>STM32F401CCUx</span></div>
            <div className="badge">CLK <span>16 MHz</span></div>
            <div className="badge">UPTIME <span>{fmtUptime(uptime)}</span></div>
        ...
        <div className="g3">
          {/* ADC */}
          <div className="card">
            <div className="label" style={{marginBottom:10}}><div className="label-dot"/>ADC1 — Channel 0 · PA0</div>
            <div className="adc-num">{adc.toString().padStart(4,"0")}<span>/ 4095</span></div>
            <div className="adc-sub">12-bit · Continuous · 3-cycle sampling</div>
            <div className="bar-track"><div className="bar-fill" style={{width:`${(adc/4095)*100}%`}}/></div>
            <div className="adc-range"><span>0</span><span>2048</span><span>4095</span></div>
            <div className="volt">{volt}<span>V</span></div>
            <div className="mini-stats">
              <div className="ms"><div className="ms-k">Min</div><div className="ms-v">{adcStats.min}</div></div>
              <div className="ms"><div className="ms-k">Max</div><div className="ms-v">{adcStats.max}</div></div>
              <div className="ms"><div className="ms-k">Avg</div><div className="ms-v">{Math.round(adcStats.sum/HIST_LEN)}</div></div>
            </div>
          </div>

          {/* PWM */}
          <div className="card">
            <div className="label" style={{marginBottom:10}}><div className="label-dot"/>TIM1 — PWM · PA8</div>
            <PWMWave duty={duty} freq={freq}/>
            <div className="pwm-stats">
              {[["Frequency",freq,"Hz"],["Prescaler",psc,""],["Period",period,""],["Pulse",pulse,""],["Timer CLK","1","kHz"],["Channel","CH1",""]].map(([k,v,u])=>(
                <div key={k} className="ps">
                  <div className="ps-k">{k}</div>
                  <div className="ps-v">{v}<span className="ps-u">{u}</span></div>
                </div>
              ))}
            </div>
          </div>
          ...
          {/* Duty */}
          <div className="card">
            <div className="label" style={{marginBottom:10}}><div className="label-dot"/>Duty Cycle</div>
            <div className="duty-wrap">
              <RingGauge value={duty} color={dutyColor} size={118}/>
              <div className="duty-big" style={{color:dutyColor}}>{duty}<span>%</span></div>
            ...
            <div className="ctrl">
              {[
                ...
                <div key={c.label}>
                  <div className="ctrl-hd">
                    ...
                    <span className={`cv${writing?" writing":""}`}>{c.val}</span>
                  </div>
                  <input
                    type="range" min={c.min} max={c.max} value={c.cur}
                    disabled={!connected}
              ...
              {writing && (
                <div style={{fontSize:9,color:"var(--amber)",fontFamily:"var(--mono)",letterSpacing:".08em"}}>
                  ↑ Menulis ke STM32…
                </div>
              )}
            </div>
        ...
        <div className="g1">
          <div className="card">
            <div className="label" style={{marginBottom:10}}><div className="label-dot"/>ADC1 Live Feed — Rolling 60 samples</div>
            <div className="chart-h">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={hist} margin={{top:4,right:8,left:-24,bottom:0}}>
                  <defs>
                  ...
                  <CartesianGrid strokeDasharray="2 4" stroke="#0f1a12" vertical={false}/>
                  <XAxis dataKey="t" hide/>
                  <YAxis domain={[0,4095]} tick={{fontSize:9,fill:"#2d4035",fontFamily:"'JetBrains Mono',monospace"}} tickLine={false} axisLine={false}/>
                  <Tooltip content={<ChartTip/>}/>
                  <Area type="monotone" dataKey="v" stroke="#22c55e" strokeWidth={1.5} fill="url(#ag)" dot={false} isAnimationActive={false}/>
                </AreaChart>
              </ResponsiveContainer>
            </div>
          </div>
        ...
        <div className="g3b">
          <div className="card">
            <div className="label" style={{marginBottom:10}}><div className="label-dot"/>System Info</div>
            <div className="si-grid">
              {[["MCU","STM32F401CCUx",true],["Family","STM32F4",true],["Package","UFQFPN48"],["Flash","256 KB"],["RAM","64 KB"],["Heap","0x200"],["Stack","0x400"],["Firmware","FW_F4 V1.28.3"],["Toolchain","STM32CubeIDE"],["Compiler","GCC -O6"]].map(([k,v,ac])=>(
                <div key={k} className="si"><div className="si-k">{k}</div><div className="si-v" style={ac?{color:"var(--green)"}:{}}>{v}</div></div>
              ))}
            </div>
          </div>
          <div className="card">
            <div className="label" style={{marginBottom:10}}><div className="label-dot"/>Pin Configuration</div>
            {[["PA0","ADC1 Ch0 input","Analog In"],["PA8","TIM1 CH1 output","Alt Func"],["NRST","Reset","Reset"],["VDD","3.3V supply","Power"],["VSS","Ground","GND"]].map(([p,d,m])=>(
            ...
            <div style={{marginTop:11,background:"var(--surface2)",border:"1px solid var(--border)",borderRadius:6,padding:"9px 11px"}}>
              <div style={{fontSize:8,color:"var(--dim)",letterSpacing:".1em",textTransform:"uppercase",marginBottom:4}}>Clock Tree</div>
              <div style={{fontFamily:"var(--mono)",fontSize:10,color:"var(--text2)",lineHeight:1.9}}>HSI 16 MHz → SYSCLK<br/>→ AHB / APB1 / APB2<br/><span style={{color:"var(--muted)"}}>All buses @ 16 MHz</span></div>
            </div>
          </div>
          <div className="card">
            <div className="label" style={{marginBottom:10}}><div className="label-dot"/>NVIC — Interrupt Config</div>
            {[["TIM1_UP_TIM10_IRQn","P0"],["SysTick_IRQn","P15"],["HardFault_IRQn","—"],["MemManage_IRQn","—"],["BusFault_IRQn","—"],["UsageFault_IRQn","—"],["PendSV_IRQn","—"],["SVCall_IRQn","—"]].map(([n,p])=>(
              <div key={n} className="irq-row">
                <div className="irq-name">{n}</div>
                <div style={{display:"flex",gap:8,alignItems:"center"}}>
                  <span style={{fontFamily:"var(--mono)",fontSize:8,color:"var(--dim)"}}>{p}</span>
                  <span style={{color:"var(--green)",fontSize:9}}>ENABLED</span>
                </div>
              </div>
            ))}
            <div style={{marginTop:10,background:"var(--surface2)",border:"1px solid var(--border)",borderRadius:5,padding:"9px 11px"}}>
              <div style={{fontSize:8,color:"var(--dim)",textTransform:"uppercase",letterSpacing:".1em",marginBottom:3}}>Priority Group</div>
              <div style={{fontFamily:"var(--mono)",fontSize:12,color:"var(--green)"}}>NVIC_PRIORITYGROUP_4</div>
            </div>
          </div>
        </div>
...
function CodeBlock({ children, lang="c" }) {
  ...
  return (
    <div style={{background:"#050807",border:"1px solid #0f1a12",borderRadius:8,overflow:"hidden",margin:"16px 0"}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"7px 14px",borderBottom:"1px solid #0f1a12",background:"#090e0b"}}>
        <span style={{fontFamily:"var(--mono)",fontSize:9,color:"var(--dim)",textTransform:"uppercase",letterSpacing:".1em"}}>{lang}</span>
        <button onClick={copy} style={{background:"none",border:"none",cursor:"pointer",color:copied?"var(--green)":"var(--dim)",fontFamily:"var(--mono)",fontSize:9,letterSpacing:".05em",transition:"color .2s"}}>
          {copied?"COPIED ✓":"COPY"}
        </button>
      </div>
      <pre style={{padding:"14px 16px",overflowX:"auto",fontFamily:"var(--mono)",fontSize:12,lineHeight:1.75,color:"#9ecfb0",margin:0}}>
...
function InfoBox({ type="info", children }) {
  const cfg = {
    info:    {bg:"rgba(34,197,94,.05)",  border:"rgba(34,197,94,.15)",  icon:"ℹ", color:"#22c55e"},
    warn:    {bg:"rgba(245,158,11,.05)", border:"rgba(245,158,11,.15)", icon:"⚠", color:"#f59e0b"},
    note:    {bg:"rgba(59,130,246,.05)", border:"rgba(59,130,246,.15)", icon:"✦", color:"#3b82f6"},
  }[type];
  return (
    <div style={{background:cfg.bg,border:`1px solid ${cfg.border}`,borderRadius:8,padding:"12px 16px",margin:"16px 0",display:"flex",gap:12}}>
  ...
  return (
    <div style={{overflowX:"auto",margin:"16px 0"}}>
      <table style={{width:"100%",borderCollapse:"collapse",fontSize:12}}>
        <thead>
          <tr>{headers.map(h=><th key={h} style={{textAlign:"left",padding:"7px 10px",borderBottom:"1px solid #0f1a12",color:"var(--dim)",fontWeight:500,letterSpacing:".07em",fontSize:9,textTransform:"uppercase",fontFamily:"var(--mono)"}}>{h}</th>)}</tr>
        </thead>
        <tbody>
          {rows.map((r,i)=><tr key={i} style={{borderBottom:"1px solid #080d09"}}>{r.map((c,j)=><td key={j} style={{padding:"7px 10px",color:j===3?"var(--green)":"var(--text2)",fontFamily:j===3||j===0?"var(--mono)":undefined,fontSize:12}}>{c}</td>)}</tr>)}
        </tbody>
      </table>
    </div>
  );
...
function SectionTitle({ id, num, children }) {
  return (
    <div id={id} style={{display:"flex",alignItems:"baseline",gap:10,margin:"32px 0 14px",scrollMarginTop:20}}>
      <span style={{fontFamily:"var(--mono)",fontSize:10,color:"var(--muted)"}}>{String(num).padStart(2,"0")}</span>
      <h2 style={{fontFamily:"var(--serif)",fontSize:22,fontWeight:300,color:"#d8e8dc"}}>{children}</h2>
    </div>
  );
}
...
function DocsPage({ onBack }) {
  ...
  useEffect(() => {
    ...
    SECTIONS.forEach(s => { const el = document.getElementById(s.id); if (el) obRef.current.observe(el); });
    ...
    <>
      ...
      <div className="docs-wrap page-enter">
        <nav className="docs-nav">
          ...
          {SECTIONS.map((s,i) => (
            <a key={s.id} className={`nav-item${active===s.id?" active":""}`}
              onClick={() => document.getElementById(s.id)?.scrollIntoView({behavior:"smooth"})}>
              <span className="nav-num">{String(i+1).padStart(2,"0")}</span>{s.title}
            </a>
          ))}
        </nav>
        <div className="docs-main">
          <div className="docs-hero">
            <div className="docs-hero-tag">Laporan Proyek · Sistem Berbasis Mikrokontroler</div>
            <h1 className="docs-hero-title">ADC-to-PWM Controller<br/>STM32F401CCUx</h1>
            <div className="prose"><p>Implementasi sistem pembacaan ADC continuous dan output PWM menggunakan STM32F401CCUx dengan HAL Library. Konfigurasi dilakukan melalui STM32CubeMX dan dikembangkan menggunakan STM32CubeIDE.</p></div>
            <div className="docs-hero-meta">
              {[["MCU","STM32F401CCUx"],["Clock","16 MHz HSI"],["ADC","12-bit Channel 0"],["PWM","TIM1 CH1 · PA8"],["IDE","STM32CubeIDE"],["Library","HAL FW_F4 V1.28.3"]].map(([k,v])=>(
                <div key={k} className="badge">{k} <span>{v}</span></div>
              ))}
            </div>
          </div>

          <SectionTitle id="pendahuluan" num="1">Pendahuluan</SectionTitle>
          <div className="prose">
            <p>Proyek ini dikembangkan sebagai implementasi sistem kontrol berbasis mikrokontroler STM32F401CCUx yang membaca sinyal analog melalui ADC dan menghasilkan sinyal PWM yang dapat dikonfigurasi. Sistem ini mendemonstrasikan penggunaan peripheral ADC1 dan TIM1 secara bersamaan dalam satu aplikasi embedded.</p>
            <p>Tujuan utama proyek adalah memahami alur kerja pengembangan firmware embedded menggunakan STM32CubeMX untuk konfigurasi peripheral dan STM32CubeIDE sebagai lingkungan pengembangan terintegrasi.</p>
          </div>
          <hr className="section-divider"/>

          <SectionTitle id="landasan" num="2">Landasan Teori</SectionTitle>
          <div className="prose">
            <p><strong>ADC (Analog-to-Digital Converter)</strong> mengubah sinyal analog menjadi nilai digital. STM32F401 memiliki ADC 12-bit dengan resolusi 4096 level (0–4095). Pada mode continuous, konversi dilakukan terus menerus tanpa intervensi software.</p>
            <p><strong>PWM (Pulse Width Modulation)</strong> adalah teknik modulasi lebar pulsa yang digunakan untuk mengatur daya rata-rata yang dikirim ke beban. Duty cycle (rasio waktu HIGH terhadap periode) menentukan nilai rata-rata sinyal.</p>
            <p><strong>Timer (TIM1)</strong> adalah peripheral advanced-control timer pada STM32F401 yang mendukung 4 kanal PWM dengan fitur dead-time insertion dan complementary output.</p>
          </div>
          <hr className="section-divider"/>

          <SectionTitle id="hardware" num="3">Spesifikasi Hardware</SectionTitle>
          <div className="spec-grid">
            {[["Mikrokontroler","STM32F401CCUx"],["Core","ARM Cortex-M4 @ 16 MHz"],["Flash Memory","256 KB"],["RAM","64 KB SRAM"],["Package","UFQFPN48"],["ADC Resolution","12-bit (0–4095)"],["ADC Input","PA0 (Channel 0)"],["PWM Output","PA8 (TIM1 CH1)"],["Supply Voltage","3.3V"],["Debug Interface","SWD (ST-Link)"]].map(([k,v])=>(
              <div key={k} className="spec-card"><div className="spec-k">{k}</div><div className="spec-v">{v}</div></div>
            ))}
          </div>
          <hr className="section-divider"/>

          <SectionTitle id="perancangan" num="4">Perancangan Sistem</SectionTitle>
          <div className="prose">
            <p>Sistem dirancang dengan arsitektur sederhana: sinyal analog pada PA0 dibaca oleh ADC1 secara continuous, kemudian nilai ADC dikonversi secara proporsional menjadi duty cycle untuk sinyal PWM pada PA8.</p>
            <p>Nilai ADC 0 menghasilkan duty cycle 0% (output LOW konstan), sedangkan ADC 4095 menghasilkan duty cycle mendekati 100% (output HIGH konstan). Pemetaan ini dilakukan melalui perhitungan linear.</p>
          </div>
          <hr className="section-divider"/>

          <SectionTitle id="peripheral" num="5">Konfigurasi Peripheral</SectionTitle>
          <Table
          ...
          <SectionTitle id="firmware" num="6">Firmware & Kode</SectionTitle>
          <div className="prose"><p>Firmware dikembangkan menggunakan HAL Library STM32F4 versi 1.28.3. Loop utama membaca nilai ADC secara continuous dan memetakannya ke duty cycle PWM.</p></div>
...
void MX_TIM1_Init(void)
{
  ...
  HAL_TIM_PWM_ConfigChannel(&htim1, &sConfigOC, TIM_CHANNEL_1);
          ...
          <InfoBox type="warn">
            Frekuensi PWM 2 Hz pada konfigurasi saat ini terlalu lambat untuk aplikasi motor atau servo (umumnya 50–20.000 Hz). Nilai Period dan Prescaler perlu disesuaikan sesuai beban yang digunakan.
          </InfoBox>
          <hr className="section-divider"/>

          <SectionTitle id="pengujian" num="7">Pengujian & Hasil</SectionTitle>
          <div className="prose"><p>Pengujian dilakukan secara fungsional untuk memverifikasi output ADC dan sinyal PWM sesuai spesifikasi desain.</p></div>
          <div className="result-grid">
            {[
              ...
              <div key={k} className="result-card">
                ...
                <div className="result-val">{v}<span className="result-unit">{u}</span></div>
          ...
          <SectionTitle id="analisis" num="8">Analisis & Pembahasan</SectionTitle>
          <div className="prose">
            <p>Sistem berhasil membaca nilai ADC secara continuous dan menghasilkan sinyal PWM dengan duty cycle yang dapat dikonfigurasi. Beberapa catatan penting dari hasil analisis:</p>
            <p><strong>Akurasi ADC:</strong> Dengan resolusi 12-bit dan VREF = 3.3V, setiap LSB mewakili tegangan <code>3.3 / 4096 ≈ 0.806 mV</code>. Noise pada pengukuran dapat diminimalisir dengan rata-rata beberapa sampel (oversampling).</p>
            <p><strong>Frekuensi PWM:</strong> Nilai 2 Hz dipilih untuk kemudahan verifikasi visual (LED akan berkedip terlihat mata). Untuk aplikasi real seperti servo, gunakan Period = 19999 dengan PSC = 15 untuk menghasilkan 50 Hz.</p>
            <p><strong>Penggunaan HAL:</strong> HAL Library menyederhanakan inisialisasi hardware namun menambah overhead dibandingkan akses register langsung (LL Library). Untuk aplikasi time-critical, pertimbangkan LL atau akses register langsung.</p>
          </div>
          <div className="two-col">
            <div className="info-card">
              <div className="info-card-title">Kelebihan Implementasi</div>
              <p>Kode bersih dan mudah dibaca. Penggunaan HAL mempercepat development. Konfigurasi CubeMX meminimalisir kesalahan inisialisasi peripheral.</p>
            </div>
            <div className="info-card">
              <div className="info-card-title">Area Pengembangan</div>
              <p>Penambahan UART untuk debug output. Mapping ADC ke duty cycle secara proporsional. DMA untuk ADC agar tidak blocking. Kalibrasi ADC internal.</p>
            </div>
          </div>
          <hr className="section-divider"/>

          <SectionTitle id="kesimpulan" num="9">Kesimpulan</SectionTitle>
          <div className="prose">
            <p>Proyek <strong>sbmproject</strong> berhasil mendemonstrasikan implementasi sistem ADC-to-PWM berbasis STM32F401CCUx. Seluruh peripheral yang dikonfigurasi — ADC1, TIM1, NVIC, dan RCC — berfungsi sesuai dengan spesifikasi yang ditetapkan dalam file konfigurasi CubeMX.</p>
            <p>Sistem ini menjadi fondasi yang solid untuk pengembangan lebih lanjut seperti kontrol motor servo, sistem otomasi berbasis sensor, atau aplikasi IoT embedded. Dengan menambahkan pemetaan ADC ke duty cycle secara dinamis, sistem dapat berfungsi sebagai kontroler analog-digital yang responsif.</p>
          </div>
          <InfoBox type="info">
            Proyek ini tersedia secara publik di <strong>github.com/dnday/sbmproject</strong> dan dapat dikembangkan lebih lanjut dengan menambahkan peripheral komunikasi (UART, I2C, SPI) atau konektivitas wireless.
          </InfoBox>
          <hr className="section-divider"/>

          <SectionTitle id="referensi" num="10">Referensi</SectionTitle>
          <div style={{display:"flex",flexDirection:"column",gap:8}}>
            {[
              ["RM0368","STM32F401xB/C/D/E Reference Manual — STMicroelectronics, 2023"],
              ...
              ["HAL","Description of STM32F4 HAL and low-layer drivers — UM1725"],
              ["CubeMX","STM32CubeMX for STM32 configuration and initialization — UM1718"],
            ].map(([id, text]) => (
              <div key={id} style={{display:"flex",gap:12,padding:"8px 0",borderBottom:"1px solid #0f1510"}}>
                <span style={{fontFamily:"var(--mono)",fontSize:10,color:"var(--green)",minWidth:60,flexShrink:0}}>[{id}]</span>
          ...
          <div style={{marginTop:40,display:"flex",justifyContent:"center"}}>
            <button className="fab-goto-docs" onClick={onBack} style={{gap:10}}>
